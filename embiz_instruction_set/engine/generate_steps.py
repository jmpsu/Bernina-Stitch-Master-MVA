...
SECTIONS = [
 ...
 dict(sid="S01", dir="01_knowledge_library",
   ...
   steps=[
    ...
    step("KB_SOURCES_INVENTORY","Knowledge-Object Source Inventory (web archive + PDFs)",
      "Inventory every source root declared in config/knowledge_sources.json (HTML sites, PDFs, existing global stores).",
      ...
      ["Was the presence of every declared knowledge source recorded?"],
      ...
      inputs=["config/knowledge_sources.json"], artifact="artifacts/kb_sources_present.json", inspect=True),
    step("KB_BUILD_LIBRARY","Build the New Knowledge Object Library",
      "Build the unified knowledge object library from HTML + PDF sources and imported global stores.",
      ...
      ["Was a real knowledge object library built with a non-empty MANIFEST on the VPS?"],
      ...
      inputs=["config/knowledge_sources.json","engine/kb_build.py"], artifact="knowledge_object_library/MANIFEST.json"),
    step("KB_PDF_EXTRACTION_VERIFY","Verify PDF Extraction Quality",
      "Verify PDFs (setup sheets, Bernina B700 manual, Inkscape/Ink-Stitch/Potrace/SVG PDFs) produced real extracted text.",
      ...
      ["Did sampled PDFs yield real text or an explicit extraction error?"],
      ["PDFs ingested (Bernina B700 manual, Inkscape/Ink-Stitch/Potrace/SVG tutorials, setup sheets)"]),
    step("KB_VISUAL_CAPTIONS","Visual Assets & Captions Store",
      "Populate the visual-caption store from images/figures/rendered SVGs for visual QA and source-vs-output comparison.",
      "Confirm knowledge_object_library/visual_captions.jsonl exists with image asset entries; record the count.",
      "python3 - <<'PY'\nimport os\np='knowledge_object_library/visual_captions.jsonl'\nn=sum(1 for _ in open(p)) if os.path.exists(p) else 0\nprint('visual caption entries:',n)\nPY",
      ["visual_captions.jsonl exists and enumerates real image/figure/SVG assets (or is empty off-VPS with that recorded)."],
      ["Does the visual-caption store enumerate real assets?"],
      ["Visual Knowledge Extraction / Visual Semantics; visual_captions store for images/diagrams/rendered SVGs/PDF figures/source-vs-output comparison"],
      artifact="knowledge_object_library/visual_captions.jsonl"),
    step("KB_RETRIEVAL_BIND","Bind Retrieval to the New Library",
      "Point the S01 retrieval interface at the newly built knowledge object library.",
      ...
      ["Does retrieval return objects from the newly built library?"],
 ...
 dict(sid="S05", dir="05_vector_validation",
   ...
   steps=[
    ...
    step("VISUAL_FIDELITY_ATTESTATION_SVG","MANDATORY Visual-Fidelity Attestation (SVG vs source)",
      "Prove — with code — the finished SVG still looks like the customer image, then state the mandated confirmation sentence.",
      ...
      ["Did fidelity_compare.py produce passing metrics for the SVG vs the source?",
       "Is the mandated confirmation sentence present verbatim and backed by those metrics?"],
      ...
      inputs=["artifacts/production.svg","the customer source image","authority/05_VISUAL_FIDELITY_MANDATE.md","engine/fidelity_compare.py"],
      artifact="artifacts/fidelity_svg.json"),
   ]),
 ...
 dict(sid="S06", dir="06_stitch_planning",
   ...
   steps=[
    step("VISUAL_FIDELITY_ATTESTATION_PRESTITCH","MANDATORY Visual-Fidelity Attestation (pre-stitch)",
      "Before any stitch plan is created from the SVG, re-prove with code that the SVG still matches the customer image, then state the mandated confirmation sentence.",
      ...
      ["fidelity_compare.py ran and passed thresholds for the SVG about to be digitized.",
       ...
       "validators/fidelity_attestation.py accepts the evidence."],
      ["Did the SVG pass fidelity comparison immediately before digitizing?",
       "Is the mandated confirmation sentence present and backed by metrics?"],
      ...
      inputs=["artifacts/production.svg","the customer source image","authority/05_VISUAL_FIDELITY_MANDATE.md","engine/fidelity_compare.py"],
      artifact="artifacts/fidelity_prestitch.json"),
 ...
 dict(sid="S08", dir="08_production_export",
   ...
   steps=[
    ...
    step("VISUAL_FIDELITY_ATTESTATION_EXPORT","MANDATORY Visual-Fidelity Attestation (SVG + PES vs source)",
      "Before export artifacts are registered, prove with code that the SVG and the rendered PES still match the customer image, then state the mandated confirmation sentence naming BOTH files.",
      ...
      ["fidelity_compare.py produced passing metrics for BOTH the SVG and the PES render vs the customer source image.",
       ...
       "validators/fidelity_attestation.py accepts the evidence."],
      ["Did BOTH the SVG and PES pass fidelity comparison vs the source?",
       "Does the mandated confirmation sentence name both .svg and .pes verbatim?"],
      ...
      inputs=["artifacts/production.svg","artifacts/production.pes","the customer source image","authority/05_VISUAL_FIDELITY_MANDATE.md","engine/fidelity_compare.py"],
      artifact="artifacts/fidelity_export.json"),
 ...
 dict(sid="S15", dir="15_production_readiness",
   ...
   steps=[
    ...
    step("DEPLOY_TO_RUNTIME","Deploy to VPS Runtime Root",
      "Ensure the system lives and runs at the configured runtime root on the VPS.",
...
def generate():
    ...
    # Prune stale generated step files no longer in the manifest (keeps the tree
    # reproducible after steps are inserted/renumbered).
    valid = {os.path.normpath(os.path.join(ROOT, m["file"])) for m in steps_index.values()}
    for sub in ("workflows", "recovery"):
