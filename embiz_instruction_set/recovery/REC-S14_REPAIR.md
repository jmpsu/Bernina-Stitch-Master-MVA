# #REC-S14_REPAIR — Recovery: Implement the Repair
<!-- generated by engine/generate_steps.py — do not hand-edit; edit DATA and regenerate. -->

## ROUTING HEADER
- STEP: #REC-S14_REPAIR
- FILE: recovery/REC-S14_REPAIR.md
- SECTION: S14 recovery — QA, Historical Learning & Continuous Improvement (never-idle loop)
- ON SUCCESS →: #REC-S14_VERIFY @ recovery/REC-S14_VERIFY.md
- ON FAILURE →: #REC-S14_ESCALATE @ recovery/REC-S14_ESCALATE.md
- ATTEMPT LEDGER: state/attempts.jsonl (key = #REC-S14_REPAIR)

## OBJECTIVES
- IMMEDIATE OBJECTIVE: implement the repair for the step recorded as `return_to` in the ledger.
...
## THE ONE ACTION
Implement exactly the repair named by the diagnosis artifact — one change, smallest blast radius, no unrelated edits. Write the repair evidence including the concrete diff and files touched.
...
## EVIDENCE TO PRODUCE
- WRITE: evidence/REC-S14_REPAIR.json
...
- HASH: `python3 engine/hash_chain.py --append '#REC-S14_REPAIR' evidence/REC-S14_REPAIR.json <SUCCESS|FAILURE>`

## ACCEPTANCE PREDICATES (ALL must be true for SUCCESS)
- [ ] The recorded `return_to` step id is identified from the ledger.
- [ ] The named repair is implemented with the smallest blast radius and its diff recorded.

## BINARY EVIDENCE QUESTIONS (answer YES or NO only)
- Q1 (mandatory): Was a hash-tagged evidence file saved to evidence/REC-S14_REPAIR.json?
...
1. State this step id: #REC-S14_REPAIR.
2. State the SUCCESS route (#REC-S14_VERIFY).
...
10. Run `python3 engine/supervisor.py --resolve '#REC-S14_REPAIR' --classification <SUCCESS|FAILURE>`
