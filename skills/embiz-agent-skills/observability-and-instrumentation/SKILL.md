---
name: embiz-observability-and-instrumentation
description: EMBIZ-adapted version of observability-and-instrumentation. Use only inside the embroidery business agent system with approval gates, evidence checks, no fake artifact claims, backups, and job-state safety.
---

# EMBIZ Adapted Skill: observability-and-instrumentation

## EMBIZ Non-Negotiable Overlay

This skill is adapted for /root/embroidery_business_agent_system.

Before applying this skill, the agent must:
- preserve human approval before customer contact
- preserve human approval before digitizing
- never claim SVG, PES, DST, EXP, INF, BMP, or stitch-ready output exists unless the file exists on disk
- never expose secrets from config, env files, logs, Cloudflare, Slack, AWS, Gmail, or Anthropic
- back up deployed files before editing
- write evidence into job folders, reports, logs, or dashboard state
- prefer local project sources before guessing
- use this skill to improve the live EMBIZ system, not as generic software advice

## Required EMBIZ Verification
...
- verification output
- remaining risks
- whether approval gates remain intact

---

# Original Source Skill Adapted Below
